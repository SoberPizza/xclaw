...
| OCR | PaddleOCR CPU (PaddlePaddle 3.x) | PaddleOCR CPU + MKL-DNN (PaddlePaddle 3.x) |
...
PaddlePaddle 统一使用 PyPI 上的 CPU 版 `paddlepaddle`（OCR ~180ms，不需要 GPU 加速），避免了 `nvidia-cudnn-cu12` 版本冲突。
...
- 图标描述使用 MiniCPM-V 2.0（`openbmb/MiniCPM-V-2`），模型自带代码（`models/icon_caption_minicpm/`）已针对 transformers 5.x 做了兼容性补丁：
  - `modeling_minicpm.py`：兼容 `is_torch_fx_available` 移除、`rope_scaling` 字段格式变更、`_tied_weights_keys` dict 格式要求
  - `modeling_minicpmv.py`：添加 `post_init()` 调用以初始化 `all_tied_weights_keys` 属性
