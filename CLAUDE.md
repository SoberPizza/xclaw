...
**截屏（眼睛）→ YOLO + PaddleOCR_mobile + SigLIP 2 感知（视觉皮层）→ 结构化 JSON（语言区）→ OS 原生键鼠操作（手）**
...
- Windows：`uv sync --extra win`（PyTorch 从 `download.pytorch.org/whl/cu121` 安装）
...
│   │   ├── icon_labels.py # 预定义图标分类标签（~100 个）
...
- **pytorch-cu121** — Windows/Linux 的 PyTorch CUDA 12.1 wheels（macOS 走 PyPI 默认的 MPS 版本）

PaddlePaddle 统一使用 PyPI 上的 CPU 版 `paddlepaddle`，避免了 `nvidia-cudnn-cu12` 版本冲突。

`[tool.uv.sources]` 中的 `marker` 确保仅在对应平台生效，macOS 不受影响。
...
- `PerceptionEngine`（`perception/engine.py`）是感知层单例，委托 `PerceptionBackend` 协议：
  - 默认后端 `PipelineBackend`（`pipeline_backend.py`）组合三个子模块：
    ...
    - `OCREngine`：PaddleOCR v5 mobile 中英双语
    - `SigLIPClassifier`：SigLIP 2 B/16 零样本图标分类，条件式调用（仅无文字覆盖/邻近的图标）
...
- 图标分类使用 SigLIP 2 B/16（`google/siglip2-base-patch16-224`），存放在 `models/icon_classify_siglip/`。
  - 文本特征在启动时预计算，推理为单次批量前向传播。
  - macOS MPS / Windows CUDA / CPU 均可直接使用，无需 monkey-patch。
...
`xclaw serve` 启动 stdio JSON-line 服务器，模型只加载一次，通过 stdin/stdout 交互。
